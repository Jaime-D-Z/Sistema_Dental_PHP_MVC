   ...
   public static function all($search = '')
{
    ...
    $sql = "
        ...
        WHERE a.is_deleted = 0 
          AND a.is_active = 1 
          AND LOWER(a.status) != 'atendido'
    ";

    if (!empty($search)) {
        $sql .= " AND (
            LOWER(pa.first_name) LIKE LOWER(:search) OR
            LOWER(pa.last_name) LIKE LOWER(:search) OR
            LOWER(CONCAT(pa.first_name, ' ', pa.last_name)) LIKE LOWER(:search)
        )";
    }
